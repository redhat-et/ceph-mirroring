...
do
    ...
    # Read AWS VPC id for this context (and strip quotes)
...
create_sg_rules() {
    ...
    EXISTS=$(aws ec2 --region ${REGIONS[from]} describe-security-groups  \
        --group-id ${SGS[from]} \
        --filters Name=ip-permission.to-port,Values=4500 Name=ip-permission.to-port,Values=4500 Name=ip-permission.cidr,Values=[${CIDRS[to]}] --query "SecurityGroups[0].GroupId" \
        --output text)
    if [ $EXISTS == "None" ]
    then
        aws ec2 --region ${REGIONS[from]} authorize-security-group-ingress \
            ...
            IpProtocol=udp,FromPort=6789,ToPort=6789,IpRanges="[{CidrIp=${CIDRS[to]}}]" \
            IpProtocol=tcp,FromPort=6789,ToPort=6789,IpRanges="[{CidrIp=${CIDRS[to]}}]" \
            IpProtocol=udp,FromPort=3300,ToPort=3300,IpRanges="[{CidrIp=${CIDRS[to]}}]" \
            IpProtocol=tcp,FromPort=3300,ToPort=3300,IpRanges="[{CidrIp=${CIDRS[to]}}]" \
            IpProtocol=udp,FromPort=6800,ToPort=7300,IpRanges="[{CidrIp=${CIDRS[to]}}]" \
            IpProtocol=tcp,FromPort=6800,ToPort=7300,IpRanges="[{CidrIp=${CIDRS[to]}}]" \
            IpProtocol=udp,FromPort=6789,ToPort=6789,IpRanges="[{CidrIp=${CIDRS[from]}}]" \
            IpProtocol=tcp,FromPort=6789,ToPort=6789,IpRanges="[{CidrIp=${CIDRS[from]}}]" \
            IpProtocol=udp,FromPort=3300,ToPort=3300,IpRanges="[{CidrIp=${CIDRS[from]}}]" \
            IpProtocol=tcp,FromPort=3300,ToPort=3300,IpRanges="[{CidrIp=${CIDRS[from]}}]" \
            IpProtocol=udp,FromPort=6800,ToPort=7300,IpRanges="[{CidrIp=${CIDRS[from]}}]" \
            IpProtocol=tcp,FromPort=6800,ToPort=7300,IpRanges="[{CidrIp=${CIDRS[from]}}]" \
